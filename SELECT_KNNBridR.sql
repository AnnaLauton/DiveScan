CREATE OR REPLACE FUNCTION SELECT_KNNBridR(
	T VARCHAR, -- Tabela T
	S TEXT, -- Coluna com as features dos elementos
	s_q INT, -- id do elemento central
	lim DOUBLE PRECISION, -- Limiar de dissimilaridade máximo
...
BEGIN
	...
	IF tipo = 'double precision[]' THEN
		...
		FOR vz_dados IN EXECUTE format(
			...
			T, S, op_dist) USING sq_f_a, s_q
		LOOP
			IF vz_dados.dist_sq <= lim THEN
				dom := true;
	...
	ELSE
		...
		FOR vz_dados IN EXECUTE format(
			'SELECT Id, %2$s AS vz_c, $1 %3$s %2$s AS dist_sq
			 ...
			 WHERE Id <> $2
			 ORDER BY $1 %3$s %2$s',
			T, S, op_dist) USING sq_f_c, s_q
		LOOP
			IF vz_dados.dist_sq <= lim THEN
				dom := true;

				FOR vz_temp IN SELECT * FROM T_R LOOP
					EXECUTE format('SELECT $1 %s $2', op_dist) INTO dist
